...
export const FloatingDock = ({
  ...
  return (
    <>
      ...
      <FloatingDockMobile onClick={onClick} items={items} className={mobileClassName} />
    </>
  );
};
  ...
  return (
    ...
    >
      {items.map((item, index) => (
        <IconContainer onClick={() => onClick(index)} mouseX={mouseX} key={item.title} {...item} />
      ))}
    </motion.div>
  );
};
  ...
  return (
    <div onClick={onClick}>
      <motion.div
        ...
        className="aspect-square rounded-full bg-gray-200 dark:bg-neutral-800 flex items-center justify-center relative h-16 w-16 bg-[rgba(248,248,248,0.01)]
